...
@Autonomous
public class RedVeryFar15 extends OpMode {
    MecanumDrive drive;
    ...
    @Override
    public void init() {
        drive = new MecanumDrive(hardwareMap, new Pose2d(72 - (16.25/2), (13 / 2) + 24, Math.toRadians(160)));
        ...
        //turnToLaunch = drive.actionBuilder(new Pose2d(72 - (16.25/2), (13 / 2) + 24, Math.toRadians(180)))
                ...
                //.build();
        driveToIntakeOne = drive.actionBuilder(new Pose2d(72 - (16.25/2), (13 / 2) + 24, Math.toRadians(175)))
                .afterDisp(20, () -> intakeUtil.intakeOn())
                .strafeToLinearHeading(new Vector2d(61,62),Math.toRadians(90), new TranslationalVelConstraint(50))
                .waitSeconds(.2)
                .strafeToLinearHeading(new Vector2d(67,64),Math.toRadians(75))
                .waitSeconds(0.5)
                .turnTo(Math.toRadians(90))
                .strafeToConstantHeading(new Vector2d(70, 66))
                .waitSeconds(0.5)
                .strafeToLinearHeading(new Vector2d(50, 12), Math.toRadians(152.5))
                .afterDisp(65, () -> intakeUtil.intakeOff())
                .build();
        driveToIntakeTwo = drive.actionBuilder(new Pose2d(52, 12, Math.toRadians(150)))
                .strafeToLinearHeading(new Vector2d(40,28),Math.toRadians(90), new TranslationalVelConstraint(50))
    ...
    public void start(){
        ...
        LauncherMotor.setVelocity(LAUNCH_TICK_VELOCITY_FAR + 475);
       ...
       // telemetry.addData("State:", state);
        ...
        switch (state){
            ...
            case SPIN_UP:
                ...
                break;
            case LAUNCH_ONE:
                if (LauncherMotor.getVelocity() >= LAUNCH_TICK_VELOCITY_FAR + 475){ // do not change this time
            ...
            case SPIN_UP_TWO:
                ...
                break;
            case LAUNCH_TWO:
                if(LauncherMotor.getVelocity() >= LAUNCH_TICK_VELOCITY_FAR + 340){
                    intakeUtil.launchStart(0.8, 1);
                    launchTimer.reset();
                    autoUtil.resetEmptyTimer();
            ...
            case SPIN_UP_THREE:
                LauncherSafetyServo.setPosition(SAFTEY_FIRING);
                LauncherMotor.setVelocity(LAUNCH_TICK_VELOCITY_FAR + 360);
                state = AutoState.LAUNCH_THREE;
                break;
            case LAUNCH_THREE:
                if (LauncherMotor.getVelocity() >= LAUNCH_TICK_VELOCITY_FAR + 360) { // todo change Time
                    intakeUtil.launchStart(0.8, 1);
                    launchTimer.reset();
                    autoUtil.resetEmptyTimer();
